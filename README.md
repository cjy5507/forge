...
| Codex | **Degraded support** | Live smoke verified `forge status`, `forge info`, explicit cross-host `forge continue`, and `forge analyze`; app-server plugin discovery/install is verified, but live hook execution is still not observed on the tested Codex runtime paths. |
...
Codex's packaged hook routing and app-server plugin-manager install path are verified, but live hook
execution has still not been observed on the tested `codex exec` and app-server turn paths.
