...
const CLAUDE_PROBE_TIMEOUT_MS = 1000;
...
function canUseClaude() {
  ...
  if (typeof claudeAvailability === 'boolean') {
...
function patternMatchesFingerprint(pattern, fingerprint) {
  if (!fingerprint) {
    return true;
  }

  const modulePaths = [
    ...(Array.isArray(fingerprint.modules) ? fingerprint.modules.map(mod => mod.path) : []),
    ...(Array.isArray(fingerprint.entryPoints) ? fingerprint.entryPoints : []),
  ];
  const directories = Array.isArray(fingerprint.directories) ? fingerprint.directories : [];

  if (pattern.startsWith('**/*.')) {
    const needle = pattern.slice(3).replace(/\*/g, '');
    return modulePaths.some(path => path.includes(needle));
  }

  if (pattern.includes('/**')) {
    const base = pattern.split('/**')[0];
    return directories.some(dir => dir === base || dir.startsWith(`${base}/`))
      || modulePaths.some(path => path === base || path.startsWith(`${base}/`));
  }

  if (pattern.includes('*')) {
    const prefix = pattern.split('*')[0];
    return directories.some(dir => dir.startsWith(prefix))
      || modulePaths.some(path => path.startsWith(prefix));
  }

  return modulePaths.includes(pattern) || directories.includes(pattern);
}
...
function heuristicDecompose(description, fingerprint) {
  ...
  const supportsParallelSplit = !['bug-fix', 'testing', 'documentation'].includes(taskType);
  const parallelizable = supportsParallelSplit && complexity >= 0.3 && areas.length >= 2;
  ...
  if (!parallelizable) {
    const comp = {
      ...
      title: 'Main task',
      areas: [...areas],
      filePatterns: areas.flatMap(area => resolveAreaFilePatterns(area, fingerprint)),
      dependencies: [],
      modelHint: 'sonnet',
    };
  ...
  if ((taskType === 'fullstack-app' || taskType === 'feature') && areas.length >= 2 && !areas.includes('shared')) {
    components.push({
      ...
      areas: ['shared'],
      filePatterns: resolveAreaFilePatterns('shared', fingerprint),
      dependencies: [],
      modelHint: 'sonnet',
    });
  }
  ...
  for (const area of areas) {
    ...
    const comp = {
      ...
      areas: [area],
      filePatterns: resolveAreaFilePatterns(area, fingerprint),
      dependencies: deps,
      modelHint: 'sonnet',
    };
