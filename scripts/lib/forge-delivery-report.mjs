...
export function summarizeHoles(holes = []) {
  const openHoles = holes.filter(hole => isOpenHoleStatus(hole.status));
  const blockers = openHoles.filter(hole => hole.severity === 'blocker');
  const majors = openHoles.filter(hole => hole.severity === 'major');
  const minors = openHoles.filter(hole => hole.severity === 'minor');
  const cosmetics = openHoles.filter(hole => hole.severity === 'cosmetic');
...
export function scopeHoleSummariesToProject(holes = [], state = {}) {
  const trackedHoleIds = Array.isArray(state?.holes)
    ? state.holes.map(value => String(value || '').trim()).filter(Boolean)
    : [];

  if (trackedHoleIds.length > 0) {
    const tracked = new Set(trackedHoleIds);
    return holes.filter((hole) => {
      const fileName = hole.filePath.split('/').pop() || '';
      const baseId = fileName.replace(/\.md$/i, '');
      return tracked.has(baseId);
    });
  }
