...
describe('forge harness hooks', () => {
  ...
  it('records host context on session restore when Claude starts the shared harness', () => {
    const cwd = makeWorkspace();
    writeState(cwd, { phase: 'develop', phase_id: 'develop', phase_name: 'develop' });

    runHook('state-restore.mjs', cwd, {}, {
      env: { CLAUDE_PLUGIN_ROOT: '/tmp/forge-plugin' },
    });

    const runtime = JSON.parse(readFileSync(join(cwd, '.forge', 'runtime.json'), 'utf8'));
    expect(runtime.host_context.current_host).toBe('claude');
    expect(runtime.host_context.previous_host).toBe('');
    expect(runtime.host_context.last_event_name).toBe('session.start');
    expect(runtime.host_context.last_resume_host).toBe('claude');
  });

  it('surfaces cross-host handoff when Codex resumes a Claude-started project', () => {
    const cwd = makeWorkspace();
    writeState(cwd, { phase: 'develop', phase_id: 'develop', phase_name: 'develop' });
  ...
  it('routes explicit forge continue through the shared resume surface', () => {
    const cwd = makeWorkspace();
    writeState(cwd, { phase: 'develop', phase_id: 'develop', phase_name: 'develop' });
    writeRuntimeState(cwd, {
      analysis: {
...
describe('state persistence round-trip', () => {
  it('write then read preserves all phases without corruption', () => {
    ...
    // Anchor the .forge dir locally so resolveForgeBaseDir() doesn't walk up
    // into an ancestor with its own .forge state (matches makeWorkspace()).
...
describe('phase-detector hook', () => {
  ...
  it('does not hijack non-forge continue text when no Forge project is active', () => {
...
describe('write-gate phase gate enforcement', () => {
  ...
  it('allows writes when all gate requirements are met', () => {
    const forgeDir = join(tmpDir, '.forge');
    writeFileSync(join(forgeDir, 'plan.md'), '# Execution Plan\n\nThis plan captures the lane graph, ordering, and verification intent for the build.\n\n## Lanes\n\n- app: implement the application lane and verify output\n\n## Execution Order\n\n1. app\n');
    writeFileSync(join(forgeDir, 'state.json'), JSON.stringify({
      mode: 'build',
      phase: 'develop',
      phase_id: 'develop',
  ...
  it('uses repo-root contracts and evidence when evaluating writes from a worktree cwd', () => {
    ...
    mkdirSync(join(forgeDir, 'evidence'), { recursive: true });
    mkdirSync(worktreeCwd, { recursive: true });
    writeFileSync(join(forgeDir, 'plan.md'), '# Execution Plan\n\nThis plan captures the lane graph, ordering, and verification intent for the build.\n\n## Lanes\n\n- shared: update shared utilities and validate the linked task brief\n\n## Execution Order\n\n1. shared\n');
    writeFileSync(join(forgeDir, 'contracts', 'api.ts'), 'export interface API {}');
    writeFileSync(join(forgeDir, 'code-rules.md'), '# Code Rules\n\n## Rules\n\n- Use TypeScript strict mode\n');
    writeFileSync(join(forgeDir, 'state.json'), JSON.stringify({
      mode: 'build',
