...
<Purpose>
Phase 4 of the Forge pipeline. The Lead Developer executes the approved execution plan.
Lane decomposition should already exist from Phase 3 (Plan); this phase turns that plan
into implemented, reviewed, and merged code. Each developer/publisher works in an isolated git worktree —
...
- state.json phase=4 / phase_id="develop"
</Use_When>
...
4. For each ready lane, Lead creates a worktree and confirms ownership:
   ...
   - Assign owner after the worktree exists:
